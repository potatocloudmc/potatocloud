...
@WebSocketDoc(
        path = "/ws/groups",
        summary = "Groups updates (WebSocket)",
        description = """
                Connect via `wss://<host>/ws/groups`.
    ...
    void broadcast() {
        ...
        try {
            String json = objectMapper.writeValueAsString(
                    new WsEnvelope<>("groups_update", groupService.findAll())
            );
            openConnections.listAll().forEach(c -> c.sendTextAndAwait(json));
        } catch (Exception e) {
            Log.error("Failed to broadcast groups_update", e);
        }
    }
